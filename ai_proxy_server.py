...
COMPONENT_SYSTEM_PROMPT = "\n".join(
    [
        ...
        "如果用户要求立体几何、空间几何、三维视角、锥体、棱柱、棱锥、平面截面，必须使用 geometry-3d，不要使用 custom-interactive。",
        ...
        'geometry-3d camera 形如 {"yaw":-35,"pitch":24,"scale":90}。',
        "geometry-3d objects 支持：point {type,id,label,x,y,z}；segment {type,from,to}；plane {type,points,label}；polyhedron {type,vertices,edges,faces,label}；cone {type,center,radius,height,label}。",
